...
function Checkout() {
  ...
  return (
    <div className="min-h-screen px-4 sm:px-6 py-12 sm:py-16 md:px-14 lg:px-24 transition-colors duration-500">
      <SEO
        title="Checkout"
        description="Secure checkout at CountrySpices. Complete your order for organic spices from Meghalaya."
        url="/checkout"
      />
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-6 sm:gap-10">
          ...
          <div className="bg-white dark:bg-charcoal-light rounded-[20px] sm:rounded-[28px] p-5 sm:p-6 md:p-8 shadow-sm border border-sand/20 dark:border-forest-light/30 h-fit sticky top-24">
            ...
            <div className="space-y-3 sm:space-y-4 font-dm">
              {cart.map((item) => (
                <div key={item._id} className="flex justify-between text-xs sm:text-sm text-forest dark:text-cream/70">
                  ...
                  <span className="text-forest dark:text-cream shrink-0">₹{item.price * item.qty}</span>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-sand/20 dark:border-forest-light/30 mt-4 sm:mt-6 pt-4 sm:pt-6">
              <div className="flex justify-between text-base sm:text-lg font-bold text-forest dark:text-cream">
                <span>Total</span>
