...
const SkeletonLoader = () => (
  <div className="min-h-screen animate-pulse px-4 sm:px-6 py-12 sm:py-16">
    <div className="mx-auto grid max-w-7xl gap-8 sm:gap-12 lg:grid-cols-2">
      <div className="h-[300px] sm:h-[400px] md:h-[550px] w-full rounded-2xl sm:rounded-3xl bg-sand/40 dark:bg-charcoal-light" />
      <div className="flex flex-col space-y-4 sm:space-y-6 pt-4 sm:pt-10">
        <div className="h-4 sm:h-6 w-24 sm:w-32 rounded-full bg-sand/40 dark:bg-charcoal-light" />
        <div className="h-8 sm:h-12 w-3/4 rounded-lg sm:rounded-xl bg-sand/40 dark:bg-charcoal-light" />
        <div className="h-6 sm:h-8 w-1/2 rounded-lg sm:rounded-xl bg-sand/40 dark:bg-charcoal-light" />
        <div className="h-24 sm:h-32 w-full rounded-lg sm:rounded-xl bg-sand/40 dark:bg-charcoal-light" />
        <div className="h-20 sm:h-24 w-full rounded-lg sm:rounded-xl bg-sand/40 dark:bg-charcoal-light" />
      </div>
    </div>
  </div>
);
...
function ProductDetails() {
    ...
    const fetchProductData = async () => {
      try {
        ...
        if (mounted && data) {
          ...
          setRelatedProducts([
            { _id: "related1", name: "Meghalaya Black Pepper", price: 299, image: "/uploads/01.jpg" },
            { _id: "related2", name: "Wild Forest Honey", price: 549, image: "/uploads/07.jpg" },
          ]);
          setReviews([
  ...
  const productLD = product ? {
    ...
    "image": product.image,
    "brand": { "@type": "Brand", "name": "CountrySpices" },
        ...
        <div className="py-6 sm:py-8 min-h-[200px]">
          ...
          {activeTab === "specifications" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-y-4 text-xs sm:text-sm text-forest dark:text-cream/70 font-dm max-w-3xl bg-cream-light dark:bg-charcoal-light p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl">
              <p><strong className="text-forest dark:text-cream block mb-0.5 sm:mb-1">Brand</strong> {product.brand || "CountrySpices"}</p>
              <p><strong className="text-forest dark:text-cream block mb-0.5 sm:mb-1">Category</strong> {product.category || "Organic Spice"}</p>
