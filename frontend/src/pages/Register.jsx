...
import { Mountain, User, Mail, Lock } from "lucide-react";
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Welcome to CountrySpices!");
      navigate("/");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 sm:px-6 py-12">
      <SEO
        title="Register"
        description="Create your CountrySpices account. Join us to shop organic spices from Meghalaya."
        url="/register"
      />
      <motion.div
      ...
      >
        <div className="text-center mb-8">
          ...
          <h2 className="mt-3 sm:mt-4 text-2xl sm:text-3xl font-fraunces text-forest dark:text-cream">Join CountrySpices</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 bg-white dark:bg-charcoal-light p-6 sm:p-8 rounded-[24px] shadow-sm border border-sand/20 dark:border-forest-light/20">
          <div>
            <label className="block text-xs font-dm font-medium text-earth dark:text-cream/60 mb-1.5">Full Name</label>
            <div className="relative">
              <User size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-earth/30" />
              <input
                type="text"
                value={name}
