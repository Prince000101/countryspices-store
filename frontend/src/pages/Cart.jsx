...
function Cart() {
  ...
  const applyCoupon = () => {
    if (coupon.toUpperCase() === "NATURE10") {
  ...
  return (
    <div className="min-h-screen px-4 sm:px-6 py-12 sm:py-16 md:px-14 lg:px-24 transition-colors duration-500">
      <SEO
        title="Shopping Cart"
        description="Review your shopping cart at CountrySpices. Organic spices from Meghalaya ready for checkout."
        url="/cart"
      />
      <div className="max-w-7xl mx-auto">
        ...
        {cart.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-16 sm:py-24 text-center">
            ...
            <p className="mt-2 sm:mt-3 text-sm sm:text-base text-forest dark:text-cream/60 font-dm">Add some organic spices to continue shopping.</p>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} className="sticky top-24 h-fit rounded-[20px] sm:rounded-[28px] bg-white dark:bg-charcoal-light p-5 sm:p-6 md:p-8 shadow-sm border border-sand/20 dark:border-forest-light/30">
              ...
              <div className="mt-6 sm:mt-8">
                ...
                <div className="flex gap-2">
                  <input value={coupon} onChange={(e) => setCoupon(e.target.value)} placeholder="NATURE10" className="w-full rounded-xl sm:rounded-2xl border border-sand/30 dark:border-forest-light/30 bg-transparent px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base outline-none text-forest dark:text-cream font-dm transition focus:border-forest" />
